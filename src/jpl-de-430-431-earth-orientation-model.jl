...
Returns the longitude (in radians) of the mean ascending node of the lunar orbit on the ecliptic, measured from the mean equinox of date 
```math
...
Returns the pole unit vector in the inertial frame ``\mathbf{p}_\mathrm{E}``, computed by precessing the pole of date with an estimated linear correction
```math
\mathbf{p}_\mathrm{E} = R_z(\zeta_A)R_y(-\theta_A)R_z(z_A)R_x(-\phi_x)R_y(-\phi_y)\mathbf{p}_\mathrm{d},
```
where ``t`` is the TDB time in Julian days from J2000.0, ``(\zeta_A, \theta_A, z_A)`` are the equatorial precession angles and ``(\phi_x, \phi_y)`` are the linear corrections.

See equation (25) in page 11 of https://ui.adsabs.harvard.edu/abs/2014IPNPR.196C...1F%2F/abstract.

See also [`pole_date`](@ref), [`Zeta`](@ref), [`Theta`](@ref), [`zeta`](@ref), [`phi_x`](@ref) and [`phi_y`](@ref).
"""
...
```
where ``t = 36,525 T`` is the TDB time in Julian days from J2000.0, ``\phi_{x0}`` is the X–axis rotation at J2000.0 (in arcseconds) and ``\frac{d\phi_x}{dt}`` is the negative obliquity rate correction (in arcseconds per year).
...
```
where ``t = 36,525 T`` is the TDB time in Julian days from J2000.0, ``\phi_{y0}`` is the Y–axis rotation at J2000.0 (in arcseconds) and ``\frac{d\phi_y}{dt}`` is precession rate correction times sine of obliquity (in arcseconds per year).
...
``\textbf{Note:}`` Lieske et al. (1977, 1979) introduces the convention that this matrix has opposite signs outside the sine terms (wrt Rx, Rz). See equation equation (4) in page 3 of https://ui.adsabs.harvard.edu/abs/1977A%26A....58....1L/abstract and equation (3) in page 283 of https://ui.adsabs.harvard.edu/abs/1979A%26A....73..282L/abstract.
...
Returns the rotation matrix from the inertial frame to the frame with pole at right ascension ``\alpha``, declination ``\delta`` and prime meridian at ``W``
```math
...
Returns the IAU 1980 nutation matrix (ESAA 1992)
```math
N = R_x(-\bar{\epsilon}(t) - \Delta\epsilon(t))R_z(-\Delta\psi(t))R_x(\bar{\epsilon}(t)),
```
where ``t`` is the TDB time in Julian days from J2000.0, ``\bar{\epsilon}`` is the mean obliquity, ``\Delta \psi`` is the nutation in longitude and ``\Delta\epsilon`` is the nutation in obliquity. All angles are in radians. 
...
Returns the matrix for celestial-to-terrestrial coordinate transformation, according to JPL DE 430/431 Earth orientation model
```math
...
Returns the Moon's angular velocity, computed by differentiating the Euler angles ``(\phi, \theta, \psi)``
```math
\begin{align*}
...
\right],
```
where ``\mathbf{r} = (x, y, z)`` is the position of the Moon relative to Earth referred to the mantle frame, ``k_{2,M}`` is the lunar potential Love number, ``m_E`` is the mass of
...
```
where ``\mathbf{r} = (x, y, z)`` is the position of the Moon relative to Earth referred to the mantle frame, ``k_{2,M}`` is the lunar potential Love number, ``m_E`` is the mass of
the Earth, ``R_M`` is the equatorial radius of the Moon, ``\mathbf{\omega}_m = (\omega_{m,x}, \omega_{m,y}, \omega_{m,z})`` is the angular velocity of the mantle in the mantle frame and ``n`` is the lunar mean motion.
